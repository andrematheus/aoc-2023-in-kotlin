 ...
 */
data class SetOfCubes(val blue: Int, val red: Int, val green: Int) {
    fun containedIn(other: SetOfCubes) =
        this.blue <= other.blue && this.red <= other.red && this.green <= other.green

    fun minimumNeeded(other: SetOfCubes): SetOfCubes =
        SetOfCubes(
    ...
    val power: Int = this.red * this.blue * this.green

    companion object {
        fun parse(line: String): SetOfCubes {
            val mapOfValues = line.split(",")
                .map { it.trim() }
                .map { it.split(" ") }
                .associateBy({ it[1] }, { it[0] })
            return SetOfCubes(
                blue = mapOfValues["blue"]?.toInt() ?: 0,
                red = mapOfValues["red"]?.toInt() ?: 0,
                green = mapOfValues["green"]?.toInt() ?: 0,
            )
        }
    }
...
data class Bag(val set: SetOfCubes)

data class Game(val id: Int, val sets: List<SetOfCubes>) {
    fun possibleWithBag(bag: Bag): Boolean =
        this.sets.all { it.containedIn(bag.set) }

    fun minimumSetNeeded(): SetOfCubes =
        this.sets.reduce(SetOfCubes::minimumNeeded)


    companion object {
        fun parse(line: String): Game {
            val (gameHeader, allSets) = line.split(":")
            val (_, gameId) = gameHeader.split(" ")
            val sets = allSets.split(";")
                .map { SetOfCubes.parse(it) }
...
fun main() {
    val theBag = Bag(SetOfCubes(red = 12, green = 13, blue = 14))

    fun part1(input: List<String>): Int {
        return input.map { Game.parse(it) }
            .filter { it.possibleWithBag(theBag) }
            .sumOf { it.id }
    }

    fun part2(input: List<String>): Int {
        return input.map { Game.parse(it) }
            .map { it.minimumSetNeeded() }
            .sumOf { it.power }
